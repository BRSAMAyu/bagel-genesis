...
| `references/autonomous-research.md` | running `autonomous_researcher` mode — choosing/executing the `discovery` (Explorer: sandboxed novel-idea generation) or `optimization` (Optimizer: benchmark-score improvement) style, framing an open direction, or beating a benchmark | `protocol_execution` mode, or a non-research run | always |
| `templates/` (fill-in-the-blank artifact templates) | producing any gated research artifact — discovery-frame/report, optimization-target/log, reproducibility-checklist, data-hygiene; copy the template and fill it rather than reconstructing the schema | non-research run with no gated artifact to produce | always |
...
- `autonomous_researcher`: scientific collaborator mode. Within the locked research direction, BAGEL may propose and run stronger ablations, robustness checks, alternative hypotheses, extra baselines, sensitivity analyses, and design amendments. Every amendment is recorded, post-result changes are labeled post-hoc, and core research identity changes still wake the user. When choosing `autonomous_researcher`, also persist `research_autonomy.objective`: **`discovery`** (Explorer — the user gives a vague direction and wants vetted *novel ideas* back; runs sandboxed with **zero blast radius** on the real project; deliverable is `.bagel/explore/discovery-report.yaml`, never commits) or **`optimization`** (Optimizer — the user wants the best score on named benchmark(s); BAGEL may improve or even replace the method, but every gain is selected on validation, the test set is touched once, and the headline binds the full Mode-1 rigor stack + referee). The two styles route to different loops, roles, and gates — load `references/autonomous-research.md` and dispatch `agents/research-explorer.md` for discovery or `agents/research-optimizer.md` for optimization.
