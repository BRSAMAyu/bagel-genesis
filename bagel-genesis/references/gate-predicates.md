...
| `no_regression_vs_green_floor` | Excellence-loop iteration N does not complete if any metric value dropped below its last-known-good value from iteration N-1's completion. The floor is recorded in `.bagel/evidence/green-floors.yaml` per iteration. Fails if any component regressed, even if current target set is all-green. |
| `metric_delta_has_evidence_artifact` | Each forward/lateral/backward metric delta in progress-deltas.yaml cites a path under `.bagel/evidence/` containing actual command output (stdout or checksum), not just the command string. Prevents hallucinated deltas. |
...
| `bar_raise_has_value_class` | Each new/raised target in a higher target set has a `why_class` field (defect_prevention \| adversarial \| growth_dimension \| churn). `churn` class requires R3+ reviewer acceptance or the bar-raise is rejected. Prevents busywork disguised as bar-raising. |
