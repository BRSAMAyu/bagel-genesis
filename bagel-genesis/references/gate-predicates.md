...
| `bar_raise_has_value_class` | Each new/raised target in a higher target set has a `why_class` field (`tighten_target`, `new_dimension`, `adversarial_lens`, `astonishing_completeness`, `stronger_research_evidence`, `defect_prevention`, `adversarial`, `growth_dimension`, or `churn`). `churn` class requires R3+ reviewer acceptance or the bar-raise is rejected. Prevents busywork disguised as bar-raising. |
